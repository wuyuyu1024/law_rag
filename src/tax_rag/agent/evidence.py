...
def _is_relevant_score(response: RetrievalResponse, primary_score: float) -> bool:
    if response.retrieval_method is RetrievalMethod.HYBRID:
...
def _is_ambiguous_score(response: RetrievalResponse, primary_score: float) -> bool:
    if response.retrieval_method is RetrievalMethod.HYBRID:
...
def grade_evidence(response: RetrievalResponse) -> EvidenceAssessment:
    ...
    if len(response.results) >= 2:
        ...
        second_score = _primary_score(response, second_result.score_map())
        if (
    ...
    if exact_match or _is_relevant_score(response, top_score):
        return EvidenceAssessment(
    ...
    if _is_ambiguous_score(response, top_score):
        return EvidenceAssessment(
            grade=EvidenceGrade.AMBIGUOUS,
